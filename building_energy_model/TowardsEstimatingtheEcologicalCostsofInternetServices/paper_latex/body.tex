...
\added{Expressing site-specific load profiles that reflect the information technology (IT) processes inside data centers (DC) is a challenge for building energy modelers. These load profiles are not trivial to produce, so data center building models (BEM) usually assume naive hour-day-week schedules that don’t align simulated operations with the weather profiles. Using these naive schedules, low fidelity BEMs are being produced for an important and growing class of buildings; negating their use for bench marking non-design day building energy performance and for predictive controls. This paper demonstrates a method towards using more representative load profiles in DC BEMs by deriving the profiles based on global usage patterns of an internet platform. It is the first attempt to illustrate the coupling of an external signal with EnergyPlus as an indicator of IT load in a data center and discuss the IT object field variables in BEMs.} 

% -------------------------------------------------------------------------------------------------------------------------------
\section*{Introduction}
% -------------------------------------------------------------------------------------------------------------------------------

\added{DCs have 10 times the power density of conventional buildings. With this density, it is essential that during the design phase designers have access to representative building energy models to size equipment and to validate jurisdictional compliance. However, these building energy models are typically put on the shelf once the facility is turned over to the operational teams. With much of the life cycle costs of a data center attributed to the use-phase, having building energy models that represent the cyber-physical dynamics of their facility can be valuable to support operational capacity constraints management decisions too.}

In the recent past, the power usage effectiveness metric ($PUE$) as defined by Equation~\ref{eq:pue} has been a key performance indicator for DC facilities used by architects and facilities operators. 
...
\deleted{In operations} \added{The $PUE$ is an intuitive metric to reason about. In consideration of the $PUE$, the mechanical inefficiencies and electrical distribution losses external to the IT equipment contribute to the difference between the numerator and denominator of Equation~\ref{eq:pue}}. In operations, the facilities and IT parameters change simultaneously together in a continuous sequence, lending to accurate real time evaluations. The challenge at design time when capacity constraints decisions are made is that the building load profile are likely not known, therefore hypothetical load schedules in the form of hour-day-week are used instead.

\deleted{Like the PUE calculations, DC building energy models are also reasoned about through coarse step loads. The step-wise inputs are sufficient for two things. First, for sizing equipment which must support the nominal DC capacity in any possible operational condition with the bounds of the basis of design. Second for producing energy models which are suitable for prescriptive compliance reviews; allowing  municipalities to normalize across different DCs in their jurisdiction.} 

These coarse load schedules misrepresent the part-load operations of the DC and inaccurately quantify total energy demands over a time period. The misrepresentation has significant impacts throughout the life of the hardware deployed in these DC facilities. For example without realistic coincident alignment of workloads and ambient conditions, the thermo-electric equipment whose energy use is dependent on workloads and ambient temperatures may be over-sized for significant portions of their operational times. \added{Yet, during these times the equipment are still allocated their nameplate power draws regardless of their lower operational points. The allocation translates to reserves on the critical power and cooling equipment which set capacity limits.}

\added{In terms of building scale, each data center can scale beyond a hundred thousand square meters with 100s of MW of power. Even with the ever growing needs for hyper-scale data centers today, there is a lack of publicly available literature which discuss coupling building energy models with IT operations to help operators reason about actual service workloads and their building's performance.}

\added{As a first illustration of a tightly coupled building energy and IT load model, this paper describes a DC building energy simulation framework that is conducive for operators to simulate their facility as operational load profiles are refined through the life of the building. The framework is based on EnergyPlus representing the physical model of the building operations and an externally implemented network traffic schedule that serves as an indicator of the IT load. Namely, the target applications for these frameworks are the hyper-scale facilities that house global services such as social networks and search engines. 

For targeted class of facilities, the significance of network traffic to DC operations is exemplified by the sudden change in people's internet usage behaviors due to the COVID-19 pandemic. Through the pandemic, DCs across the globe have seen a disruptive shift in user traffic patterns that were not known just weeks prior to the first stay-in-place orders. This type of change in user traffic directly translates to changes in the data center operational loads.  Given this timely example, it is imperative that DC operators be able to simulate their facilities to verify that their infrastructure can handle these types of shifts in traffic.} 

\added{In this work, wide area network traffic bandwidth is demonstrated as a proxy for IT loads in data center buildings. However, any technique that characterizes a fine grain coupling between the IT workloads and the building systems would be valuable for hyper-scale DCs the running online platforms such as social media sites, search engines, or public cloud services. These platforms are typically distributed through out the world for redundancy and performance objectives for their global user base that can be characterized by considering the systems from the top down. Such a top down network assessment method that considers the global distribution of DCs is presented in this article.} 


The developed simulations allow for more realistic evaluations of DC energy use. These more realistic evaluations are useful for two things. (1) operators can use the realistic cyber-phsyical models to simulate the DC building performance under perceived usage changes. (2) by exposing the facility's power and cooling equipment headroom for non-design day conditions they can enable DC operators to opportunistically oversubscribe the IT loads. Such a oversubscribing scheme can be implemented by extending the electrical over-subscription \added{leading to free capacity} schemes presented by Li \added{when coupled with predicative modeling techniques \citep{Li18}}.

\added{The contribution of this work is two-fold. First, it provides a demonstration for characterizing data centers and IT equipment in EnergyPlus. Specifically EnergyPlus' external interface feature is used to integrate a network profile scripted in Python to reset the power demand of the IT objects. Second, by using a top down method for deriving the workload profile from network traffic, this work provides a perspective for reasoning about temporal profiles of the workloads that hyper-scale data centers must support.}

% Deleted b/c it is repetitive
...
In the next section, the authors discuss similar works on data center energy models. Then discussed in the subsequent section, Methodology, are their methods for more expressively representing load profiles in EnergyPlus data center models for a network of data centers. The model's results are then presented in the Results section followed by a Discussion section in which the authors reason about the article's results and claims. The paper then concludes by highlighting the contribution and identifying opportunities for extending this work. 
...
In this section a dynamic workload-profile based energy simulator is introduced and  described in detail. First, an overview of this work's cross platform software integration is presented. Then details about each of the two main components, namely EnergyPlus and Python based traffic models, are discussed.  As shown in Figure~\ref{img_ep_py}, the proposed simulator uses Python to reset the DC IT load for each time-step. In the subsequent modeling time-step, the EnergyPlus models run with the externally revised variables. The basic process loop of the simulator is illustrated in Figure~\ref{fig:dyna-bem}.
...
The variations in the relative energy demands between the two sets of simulations can be attributed to the CPU load profiles as illustrated in Figure~\ref{fig:cpu_comps}. The line curves are indicative of CPU power demands of each DC analyzed through the RL simulations. In the figure, the x-axis indicates the hour of the year and the y-axis indicates the CPU load for each respective DC. \deleted{Examining}\added{The CPU profiles for each DC show that} their \deleted{values appear quite}\added{average values} are differ from one another as a result resetting the loads with the traffic profile. Carrolton and Ashburn appear to have  \deleted{strong}\added{noticeable} seasonal patterns. Amsterdam and Singapore DCs show irregular oscillations in demand throughout the year\added{, but have a tight band}. The San Francisco DC has a constant line. This constant demand is due to the load not varying much from the peak capacity in the traffic profile that was used for the site. 

In contrast to the distinct CPU load profiles of the RL simulations as discussed above, the gray band in Figure~\ref{fig:cpu_comps} shows the NoRL model's CPU load profile. The band is composed of uniform loads that cycle daily based on the statically configured schedule. The uniformity of the loads at each DC leads to the CPU loads at all of them being identical to each other; therefore all the DCs have an overlapping CPU profile for this simulation.
...
In the next section, the extensibility of the traffic based building energy model towards predicative scenarios and the potential for using this framework to exploit 'free capacity' are discussed.  
...
\added{An obstacle data center operators face before using building energy models to make operational decisions is that working with these models requires deep knowledge about the software implementation of the models. Nonetheless, in many cases cursory working knowledge of the models coupled with other tools they are familiar can unblock the obstacles. Specifically, our method of iterating on the load schedule external to the BEM proved to be quite valuable as we could simply use Python logic to scale and reiterate the models.

As the most valuable insights for using EnergyPlus' external interface, operations teams can simulate perceived future changes to their work loads without direct interaction with EnergyPlus. As mentioned above; predicative modeling is feasible when using network traffic as the proxy for IT loads. Network traffic has been shown to be predictable within reasonable accuracy for internet services by Linkedin \citep{zhuang15}. Figure~\ref{fig:arima_bem} shows the predicative models for four of the data center studied in the simulations in this work. Such future projections can be simply passed to EnergyPlus and assessments about the building performance can be made as we have demonstrated.}

\input{building_energy_model/TowardsEstimatingtheEcologicalCostsofInternetServices/paper_latex/content/arima_all}

In this research the network traffic serves as a proxy for CPU utilization. In practice the network traffic and CPU have complex correlations that can't be generalized with the presented approach. In production DC environments representative correlations can be obtained between network traffic, the IT workloads that the traffic instantiate, and the workload's CPU demand. However, proving such correlations is out of scope of research. \added{Nonetheless, every data center network and service platform pair will be subject to its own traffic models. As mentioned in the Introduction, this framework lends itself to other techniques that can characterise data center workloads.}
...
\added{Now, we provide some discussions on the claim for opportunistic 'free capacity'. The free capacity is in terms of supplying more power to the IT equipment then their design power capacity value (ie over-subscribe). Design capacities are typically set on utility power reservations and back-up power generator reservations required to support expected (peak values in naive cases) cooling equipment and IT conditions. This means that anytime when the cooling plant power reservation exceeds the part load power demand; power gets stranded and the headroom in power infrastructure provisioned for the cooling equipment can be safely supplied to IT equipment. The rerouting of the available power increases the supportable IT load and essentially yields 'free capacity' beyond the design values of the IT load.}

Our claim leans one of the authors professional experiences at several of largest DC operators in the world. For the readers posterity, Li provides a comprehensive overview about principals of over-subscription that opportunistically exploit capacity headroom based on electrical topologies \cite{Li18}. Using the quality of service (QoS) based service differentials described by Li, free-capacity can be used to opportunistically schedule additional IT loads that are not critical and can be pre-emptied. 

As an example of free capacity, consider a design day dry-bulb temperature of 105$^{\circ}$F. When a peak in IT workload occurs on a more favorable day, say 95$^{\circ}$F, the computer room air conditioner heat rejection equipment needs 50\% of the designed power value \citep{liebert20}. With a slight modification in the electrical infrastructure, this difference can opportunistically be allocated to IT workloads. However slight the the electrical modification, the decision is most cost effectively made during the design phase of the the facility. This influence on the design presents a case for using the fine grained load aware model during the design phase as-well.

\added{Opportunistically using 50\% of the provisioned power from the cooling equipment to power the IT equipment requires some further insights about DC operation. First, the 50\% claim is based on nameplate full load amps (FLA) for two different dry-coolers (Liebert Drycooler D-466 and D880). that are matched with a 105kW indoor computer room air conditioner (Liebert Indoor Model 105). One of the dry-coolers is rated at 95$^{\circ}$F and the other is rated at 105$^{\circ}$F \citep{liebert20}. The FLA for these units are 14 Amps and 28 Amps respectively. Physically this translates to breaker sizes, wires sizes, utility power feed reservations, and back-up generator reservations decisions being made using this value. Second, with the claim it is assumed that the power draw has the same proportional scaling with loads cross both sets of dry-cooler plants. Naively this can be achieved with redundant cooling plants optimized for a ambient temperature threshold where control logic can select the right plant to operate. In all cases over subscription is an important consideration for a building class where its utility is measured by the power delivered to IT equipment use. Anytime when the stranded power can be re-purposed from auxiliary infrastructure (such as chillers, cooling towers, and transformers) to the IT equipment, it leads to compelling return on investment analysis.
...
This article makes a case for using externally implemented IT load profiles in EnergyPlus based building energy models for data centers. Using this hybrid approach makes BEMs more accessible and useful for building operators to assess the performance of their facilities as changes in load profiles become known. The details for expressing this hybrid model in EnergyPlus are provided in this work for others to replicate it towards similar models. The presented details also address several other IDF object fields that can be changed to more accurately represent data centers IT. 

The proposed model with the externally defined load profile allows for more expressive characterization of IT loads. In the presented case, network traffic was used due to it being conducive to forecasting based on historical time-series. However, any other time-series signal that can characterize computation workloads suffices. By representing the dynamics of the IT load, BEMs can evaluate the operational demands based on coincident weather. As most data centers are sized for extreme design days, BEMs can become an integral part of capacity and constraints management for data centers. Managing capacity and constraints in such a dynamic way, will provide free capacity opportunities beyond the nominal provisioned power of the data center. 

\section{Future Work}
As the first piece of future work, this method needs to be validated with real data centers. The second piece of future work should look at applying this load aware model for control optimization of data center cooling plants and network traffic load balancing. 



\deleted{There are two functional benefits fr resetting the IT load values outside of the IDF file. First, defining load values for each hour in the IDF file is toilsome and error prone. The toil of entering values for each simulation time-step would require 8,760 entries in the file. This increases the chance of introducing errors into the simulation. Second, the external interface allows more sophisticated logic to control the time-step variables. In this article, only the IT load factors were reset, none-the-less the same interface can reset many other simulation run-parameters with similar logic. An example of a feedback logic is reinforcement learning, which can be framed to globally optimize the systems and is an topic for future research.}

% The traffic profiles used in this article are representative of a globally distributed service’s user facing workloads and they suffice for purposes of demonstrating the dynamics of DC operational loads. Each real-world service will have a unique profile resulting from a combination of critical and opportunistic back-end workloads as characterised by the QoS values.

% Figure~\ref{fig:lang2dc} aggregates all the languages to provide an overall building simulation. Future work that need to assess a service platform can create multiple ITE objects in the EnergyPlus thermal zone and quantify the energy demands for each in isolation.
