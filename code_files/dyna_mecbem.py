...
class agent(object):
    ...
    def expand_traffic(self):
        ...
        '''Todo: Fix the Nan Values'''
        '''Results in a dictionary indexed by Site'''
        dmd = {}
        for site in self.Sites:
            ...
            for i in range(1,8760):
                if math.isnan(traffic_list[int(i/24)]):
                    dmd[site].append(dmd[site][i-1])
                else:
...
def main():
    print("Straight outta mecbem")
    traffic_file_1 = '../../traffic/data/unit_testing.csv'
    #traffic_file_1 = '../../traffic/data/train_1.csv'
   
    # Agent = agent('Eplus-dc_golden-v0','en','../../traffic/data/unit_testing.csv')
    
    # singapore = agent('Eplus-dc_Singapore-v1','en',traffic_file_1)
    # dmd = singapore.expand_traffic()
    # print(singapore.schedule_run('Singapore', dmd=dmd))

    # ashburn = agent('Eplus-dc_Ashburn-v1','en', traffic_file_1)
    # dmd = ashburn.expand_traffic()
    # ashburn.schedule_run('Ashburn', dmd=dmd)

    # carrolton = agent('Eplus-dc_Carrollton-v1','en', traffic_file_1)
    # dmd = carrolton.expand_traffic()
    # carrolton.schedule_run('Carrollton', dmd=dmd)

    # haarlem = agent('Eplus-dc_Haarlem-v1','en',traffic_file_1)
    # dmd = haarlem.expand_traffic()
    # print(haarlem.schedule_run('Haarlem', dmd=dmd))

    sfo = agent('Eplus-dc_SanFrancisco-v1','en',traffic_file_1)
    dmd = sfo.expand_traffic()
    print(sfo.schedule_run('San Francisco', dmd=dmd))
    # print(sfo._traffic()['San Francisco'])
    # print(pd.DataFrame(sfo.expand_traffic()['San Francisco']).sum())
    # print(sfo.expand_traffic()['San Francisco'])
