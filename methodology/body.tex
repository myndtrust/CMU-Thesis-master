...
\section{Information Technology Equipment}
        ...
        Steel racks host the server assemblies. Figure~\ref{bcu_rack} and Figure~\ref{goog_rack} are examples of hyper-scale racks. At the rack level these dis-aggregated servers can be mixed and matched where high speed communication is hermetic to the rack. Alternately the entire rack can be of a single type, bridged by high speed communications networks across local areas. The choice for the rack arrangement is reasoned about as performance objectives for specific workloads that the rack will be susceptible to.
